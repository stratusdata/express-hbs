...
function _express3(filename, options, cb) {
  ...
  function render(template, locals, layoutTemplate, cb) {
    var opts = {};
    Object.keys(self._options.templateOptions).forEach(function(id) {
      opts[id] = self._options.templateOptions[id];
    })
    opts.data = locals;
    var res = template(locals, opts);
    async.done(function(values) {
      Object.keys(values).forEach(function(id) {
        res = res.replace(id, values[id]);

        // Hacky but we need to replace the async data in blocks as well, which
        // won't be accomplished when calling replace against the generated html.
        var blocks = self.blocks;
        Object.keys(blocks).forEach(function(name) {
          Object.keys(blocks[name]).forEach(function(n) {
            blocks[name][n] = blocks[name][n].replace(id, values[id]);
          });
        });  
      });
      ...
      locals.body = res;
      opts.data = locals;
      var layoutResult = layoutTemplate(locals, opts);
      async.done(function(values) {
        Object.keys(values).forEach(function(id) {
          layoutResult = layoutResult.replace(id, values[id]);

          // See above - replace async values in blocks, too.
          var blocks = self.blocks;
          Object.keys(blocks).forEach(function(name) {
            Object.keys(blocks[name]).forEach(function(n) {
              blocks[name][n] = blocks[name][n].replace(id, values[id]);
